  ...
  try {
    ...
    const response = await fetch("https://api.mercadopago.com/v1/payments", {
      ...
      headers: {
        ...
        "X-Idempotency-Key": Math.random().toString(36).substr(2, 9), // Prevents duplicate payments
      },
    ...
    if (!response.ok) {
      console.error("Mercado Pago API error:", paymentResult);
      throw new Error(paymentResult.message || "Error processing payment");
    }
